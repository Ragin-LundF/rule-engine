...
object TextRegexOperator {
    fun compile(ruleId: String?, cond: ConditionAst, fieldId: FieldId): CompiledExpression {
        val literal = cond.value as? StringLiteral ?: throw CompilationException(ruleId, "Operator 'regex' expects string literal pattern for field '${cond.field}'")
...
object TextInOperator {
    fun compile(ruleId: String?, cond: ConditionAst, fieldId: FieldId, def: FieldDefinition, registry: NormalizerRegistry): CompiledExpression {
        return when (val lit = cond.value) {
            is ListLiteral -> {
                val set = lit.items.map { (it as? StringLiteral)?.value ?: throw CompilationException(ruleId, "Expected string items in list") }.toSet()
...
object TextComparisonOperators {
    fun compile(ruleId: String?, op: String, cond: ConditionAst, fieldId: FieldId, def: FieldDefinition, registry: NormalizerRegistry): CompiledExpression {
        val literal = cond.value as? StringLiteral ?: throw CompilationException(ruleId, "Expected string literal for text field '${cond.field}'")
        var v = literal.value
        for (n in def.normalizers) v = registry.get(n).normalize(v)
        val expected = v
