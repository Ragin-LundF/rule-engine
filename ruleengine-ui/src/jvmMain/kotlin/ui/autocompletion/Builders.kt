...
/** Build completion items based on DSL cursor context. */
public fun buildContextualCompletions(
    ...
    return when (context.section) {
        ...
        DslSection.WHEN -> buildWhenCompletions(context = context, schema = schema)
        DslSection.THEN -> buildThenCompletions(context = context, actionSchema = actionSchema)
    }
}
...
private fun buildWhenCompletions(context: DslCursorContext, schema: FieldSchema?): List<CompletionItem> {
    return when {
...
private fun buildThenCompletions(context: DslCursorContext, actionSchema: ActionSchema?): List<CompletionItem> {
    return if (context.afterAction == null) buildActionNameCompletions(actionSchema = actionSchema)
    else buildActionArgCompletions(actionName = context.afterAction, actionSchema = actionSchema)
}
...
private fun buildOperatorCompletions(fieldName: String, schema: FieldSchema?): List<CompletionItem> {
    ...
    val operators =
        if (def.operators.isNotEmpty()) def.operators.map { it.value } else defaultOperatorsForType(def.type)
...
public fun buildAllCompletions(schema: FieldSchema?, actionSchema: ActionSchema?): List<CompletionItem> {
    return buildList {
        ...
        actionSchema?.actions?.forEach { (name, def) ->
            val argPh = def.argTypes.joinToString(" ") { t ->
                when (t) {
                    ActionArgType.INTEGER -> "0"; ActionArgType.DECIMAL -> "0.0"; else -> "\"value\""
                }
            }
